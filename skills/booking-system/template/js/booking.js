/**
 * Booking modal — generic template
 *
 * From the AutoWebDev booking-system skill. Drop this in /js/booking.js,
 * customize the SHOP_CONFIG block below, and any element with [data-book]
 * opens the modal. Optional [data-service="<key>"] pre-selects a service.
 *
 * Flow: service → (duration?) → date → time → details → confirm + .ics
 *
 * The matching server-side proxy is in /api (see SKILL.md).
 */
...
(function () {
  ...
  // ════════════════════════════════════════════════════════════════════
  // SHOP_CONFIG — the ONLY block you need to edit per shop
  // ════════════════════════════════════════════════════════════════════
  const SHOP_CONFIG = {
    // Display name + contact fallbacks shown in the modal copy.
    shopName:     'Voorbeeldsalon',
    phoneDisplay: '06 12 34 56 78',
    phoneE164:    '+31612345678',           // tel: links + 'bel/WhatsApp' fallback copy
    domain:       'voorbeeldsalon.nl',      // used in ICS UID + Cal.com metadata.source
    addressICS:   'Hoofdstraat 1, 1234 AB Plaatsnaam',
    timeZone:     'Europe/Amsterdam',
    language:     'nl',

    // Open hours per weekday. 0 = Sunday … 6 = Saturday. null = closed.
    // Must match Cal.com → Settings → My Availability exactly.
    hours: {
      0: null,
    ...
    // Each service maps to one or more durations. The Cal.com event-type
    // slug is computed as `${key}-${duration}`. So `intuitief` + 60 →
    // create a Cal.com Event Type with slug `intuitief-60`.
    //
    // `fixed: true` skips the duration step (service has one length).
    // `free: true` cosmetic only — displays "gratis" instead of price.
    services: [
      { key: 'sample-60', title: 'Voorbeelddienst 60 min', desc: 'Korte omschrijving',   durations: [60, 90] },
      { key: 'sample-30', title: 'Voorbeelddienst 30 min', desc: 'Vaste duur, kort',     durations: [30], fixed: true },
      { key: 'intake',    title: 'Intakegesprek',          desc: 'Vrijblijvend (gratis)', durations: [30], fixed: true, free: true },
    ],

    // Display prices per duration. Keys are minutes.
    price: { 30: '€37,50', 60: '€65', 90: '€90' },
  };
  // ════════════════════════════════════════════════════════════════════

  // Internal aliases (do not edit below this line for normal customization)
  const SHOP_HOURS = SHOP_CONFIG.hours;
  const SERVICES = SHOP_CONFIG.services;
  const PRICE = SHOP_CONFIG.price;
  const FREE_KEYS = new Set(SERVICES.filter(s => s.free).map(s => s.key));
  ...
  const buildICS = ({ summary, description, location, start, durationMin }) => {
    ...
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//' + SHOP_CONFIG.domain + '//Booking//' + SHOP_CONFIG.language.toUpperCase(),
      'BEGIN:VEVENT',
  ...
  function viewDate() {
    ...
    return `
      ...
      <p class="text-sm text-ink-muted mb-6">Ma t/m vr · 08:00 — 18:00. Weekend en feestdagen op aanvraag.</p>
  ...
  async function wireTime(root) {
    ...
    if (!eventTypeId) {
      status.textContent = '`Online boeken nog niet geactiveerd — bel of WhatsApp ${SHOP_CONFIG.phoneDisplay}.`';
    ...
    return `
      ...
      <form class="space-y-3" data-form>
        ...
        <div>
          ...
          <textarea name="notes" rows="3" class="w-full bg-bg-deep border border-border-subtle rounded-[12px] px-4 py-3 text-ink-primary placeholder:text-ink-faint focus:outline-none focus:border-gold transition-colors" placeholder="Wat speelt er? Eerste keer? Een specifieke vraag?"></textarea>
        </div>
  ...
  function viewConfirm() {
    ...
    return `
      <div class="text-center py-2">
        ...
        <p class="text-ink-muted mb-7 max-w-md mx-auto">`Je krijgt een bevestiging in je inbox. Tot ziens bij ${SHOP_CONFIG.shopName}.`</p>
  ...
  function wireConfirm(root) {
    root.querySelector('[data-ics]')?.addEventListener('click', () => {
      ...
      const ics = buildICS({
        ...
        description: `Afspraak van ${state.duration} minuten bij ${SHOP_CONFIG.shopName}.`,
