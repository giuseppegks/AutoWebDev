/**
 * Booking modal — Kanta Massage
 *
 * Architecture (per /skills/booking-system/SKILL.md):
 *   1. On open: fetch /api/event-types, build slug → ID map
 *   2. Step "service":   user picks one of the Thai massage forms
 *   3. Step "duration":  multiple durations available (skipped for fixed-duration services)
...
(function () {
  ...
  // ─── Shop config ──────────────────────────────────────────────────
  // One place to update shop-specific data. Used throughout the modal.
  const SHOP_CONFIG = {
    shopName: 'Kanta Massage',
    practitioner: 'Pimchaya',
    phoneDisplay: '06 81 62 02 17',
    phoneE164: '+31681620217',
    email: 'kantamassage@gmail.com',
    domain: 'kantamassage.nl',
    addressICS: 'Danielsplein 3D, 6543 AB Nijmegen',
  };

  // ─── Configuration ────────────────────────────────────────────────
  const SHOP_HOURS = {
    // 0=Sun … 6=Sat. null = closed. Slots are quoted in local Europe/Amsterdam time.
    // Kanta Massage: ma-wo 09:00–21:00, do gesloten, vr-zo 09:00–21:00.
  ...
  // Each service has up to several durations. The slug is what gets created
  // as an Event Type in Cal.com. Update if the client renames things.
  const SERVICES = [
    { key: 'thai',           title: 'Traditionele Thaise massage', desc: 'Klassieke Thaise techniek met strekken en drukpunten', durations: [30, 60, 90, 120, 180] },
    { key: 'olie',           title: 'Olie ontspanningsmassage',    desc: 'Zachte, vloeiende strijkbewegingen met warme olie',  durations: [60, 90] },
    { key: 'hotstone',       title: 'Hotstone massage',            desc: 'Warme basaltstenen op spierknopen en meridianen',     durations: [90], fixed: true },
    { key: 'kruidenstempel', title: 'Kruidenstempel massage',      desc: 'Warme Thaise kruiden­stempels op het lichaam',        durations: [90], fixed: true },
    { key: 'deeptissue',     title: 'Deep tissue massage',         desc: 'Diepe druk op vastzittend bindweefsel en spieren',    durations: [60, 90] },
    { key: 'antimigraine',   title: 'Anti-migraine massage',       desc: 'Gerichte druk op hoofd, nek en schouders',            durations: [60] },
    { key: 'zwangerschap',   title: 'Zwangerschapsmassage',        desc: 'Veilige, zachte massage in zijligging',               durations: [60] },
  ];

  // Pricing per duration in minutes. Values for Traditional Thai per the
  // public price list; oil, deep tissue, anti-migraine, pregnancy use the
  // same scale (verify with Pimchaya before live).
  const PRICE = {
    30: '€50',
    60: '€60',
    90: '€85',
  ...
  // Service-specific overrides (hotstone & kruidenstempel are €105 / 90 min).
  const PRICE_OVERRIDES = {
    'hotstone-90':       '€105',
    'kruidenstempel-90': '€105',
  };
  const FREE_KEYS = new Set();

  // Slugs are computed as `${service.key}-${duration}`, e.g. "thai-60".
  const slugFor = (key, dur) => `${key}-${dur}`;
  const priceFor = (key, dur) => PRICE_OVERRIDES[slugFor(key, dur)] || PRICE[dur] || '';
  ...
  function viewService() {
    const cards = SERVICES.map(s => {
      let priceLine;
      if (FREE_KEYS.has(s.key)) {
        priceLine = 'gratis';
      } else if (s.fixed) {
        priceLine = priceFor(s.key, s.durations[0]);
      } else {
        const first = priceFor(s.key, s.durations[0]);
        const last = priceFor(s.key, s.durations[s.durations.length - 1]);
        priceLine = `${first} — ${last}`;
      }
      return `
  ...
  function viewDate() {
    ...
    return `
      ...
      <p class="text-sm text-ink-muted mb-6">Ma t/m wo en vr t/m zo · 09:00 — 21:00. Donderdag gesloten.</p>
    ...
    return `
      ...
      <form class="space-y-3" data-form>
        ...
        <div>
          ...
          <textarea name="notes" rows="3" class="w-full bg-bg-deep border border-border-subtle rounded-[12px] px-4 py-3 text-ink-primary placeholder:text-ink-faint focus:outline-none focus:border-gold transition-colors" placeholder="Wat speelt er? Eerste keer bij ${SHOP_CONFIG.practitioner}? Specifieke wens?"></textarea>
        </div>
  ...
  function viewConfirm() {
    ...
    return `
      <div class="text-center py-2">
        ...
        <p class="text-ink-muted mb-7 max-w-md mx-auto">Je krijgt een bevestiging in je inbox. Tot ziens bij de praktijk aan het ${SHOP_CONFIG.addressICS}.</p>
  ...
  function wireConfirm(root) {
    root.querySelector('[data-ics]')?.addEventListener('click', () => {
      ...
      const ics = buildICS({
        ...
        description: `Afspraak van ${state.duration} minuten bij ${SHOP_CONFIG.practitioner}.`,
  ...
  window.KM_Booking = { open: openModal, close: closeModal };
})();
