 ...
 * pre-selects a service (used by per-treatment "Boek deze" buttons).
 */
...
(function () {
  ...
  const SHOP_CONFIG = {
    shopName:     'HomeMas',
    phoneDisplay: '06 28 80 91 49',
    phoneE164:    '+31628809149',
    email:        'homemasmassage@gmail.com',
    domain:       'homemas.nl',
    addressICS:   'Tweede Walstraat 85, Nijmegen',
  };

  // ─── Configuration ────────────────────────────────────────────────
  const SHOP_HOURS = {
    // 0=Sun … 6=Sat. null = closed. Slots are quoted in local Europe/Amsterdam time.
    // PLACEHOLDER: opening hours not published on homemas.nl — Nienke werkt op afspraak;
    // verify exact dagen/uren met haar voor livegang. Aanname: ma–za 09:00–18:00.
    0: null,
  ...
  const SERVICES = [
    { key: 'tuina',       title: 'Chinese massage (Tui Na)',  desc: 'Traditionele Chinese massage, meridianen & energie', durations: [60, 90] },
    { key: 'voetreflex',  title: 'Voetreflexmassage',         desc: 'Drukpunten onder de voet, hele lichaam',             durations: [45, 60] },
    { key: 'hotstone',    title: 'Hotstonemassage',           desc: 'Warme basaltstenen, yang en doorstroming',           durations: [60, 90] },
    { key: 'ontspanning', title: 'Ontspanningsmassage',       desc: 'Doorbloeding, soepelheid, rust',                     durations: [45, 60] },
    { key: 'taping',      title: 'Tapen',                     desc: 'Medical taping ter ondersteuning',                   durations: [30], fixed: true },
    { key: 'pedicure',    title: 'Pedicurebehandeling',       desc: 'Volledige verzorging van nagels, eelt en kloven',    durations: [60], fixed: true },
  ];

  // Prijzen op basis van homemas.nl tarievenpagina (massage 30/45/60/90 min).
  // Pedicure 60 min = €35 standaard. Tapen 30 min = €32 (zelfde tarief als 30-min massage).
  const PRICE = { 30: '€32', 45: '€43', 60: '€55', 90: '€77' };
  const PRICE_OVERRIDE = { pedicure: { 60: '€35' } };
  const FREE_KEYS = new Set();

  // Slugs are computed as `${service.key}-${duration}`, e.g. "tuina-60".
  const slugFor = (key, dur) => `${key}-${dur}`;
  const priceFor = (key, dur) => (PRICE_OVERRIDE[key] && PRICE_OVERRIDE[key][dur]) || PRICE[dur] || '—';
  ...
  const buildICS = ({ summary, description, location, start, durationMin }) => {
    ...
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//homemas.nl//Booking//NL',
      'BEGIN:VEVENT',
      `UID:${Date.now()}@homemas.nl`,
      `DTSTAMP:${dt(new Date())}`,
      `DTSTART:${dt(start)}`,
      `DTEND:${dt(end)}`,
  ...
  function viewService() {
    const cards = SERVICES.map(s => {
      const priceLine = FREE_KEYS.has(s.key)
        ? 'gratis'
        : s.fixed
          ? priceFor(s.key, s.durations[0])
          : `${priceFor(s.key, s.durations[0])} — ${priceFor(s.key, s.durations[s.durations.length - 1])}`;
      return `
      ...
      <div class="grid sm:grid-cols-2 gap-3">
        ${svc.durations.map(d => `
          <button class="text-left bg-bg-deep border border-border-subtle rounded-[16px] p-6 hover:border-gold/60 hover:bg-bg-elevated transition-all" data-dur="${d}">
            ...
            <p class="text-[11px] tracking-kicker uppercase text-gold font-semibold">${priceFor(state.serviceKey, d)}</p>
          </button>
        `).join('')}
      </div>
  ...
  function viewDate() {
    ...
    return `
      ...
      <p class="text-sm text-ink-muted mb-6">Maandag t/m zaterdag · op afspraak. Zondag en feestdagen op aanvraag.</p>
  ...
  async function wireTime(root) {
    ...
    if (!eventTypeId) {
      status.textContent = 'Online boeken nog niet geactiveerd voor deze massage — bel of WhatsApp 06 28 80 91 49.';
    ...
    try {
      ...
      status.textContent = 'Kon beschikbaarheid niet ophalen. Probeer opnieuw of bel 06 28 80 91 49.';
      console.warn(e);
    }
  }
    ...
    return `
      ...
      <form class="space-y-3" data-form>
        ...
        <div>
          ...
          <textarea name="notes" rows="3" class="w-full bg-bg-deep border border-border-subtle rounded-[12px] px-4 py-3 text-ink-primary placeholder:text-ink-faint focus:outline-none focus:border-gold transition-colors" placeholder="Wat speelt er? Eerste keer bij Nienke? Vraag voor de intake?"></textarea>
        </div>
  ...
  function wireDetails(root) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        errEl.textContent = err.message || 'Kon de boeking niet vastleggen. Probeer opnieuw of bel 06 28 80 91 49.';
  ...
  function viewConfirm() {
    ...
    return `
      <div class="text-center py-2">
        ...
        <p class="text-ink-muted mb-7 max-w-md mx-auto">Je krijgt een bevestiging in je inbox. Tot ziens bij de praktijk aan de Tweede Walstraat 85.</p>
  ...
  function wireConfirm(root) {
    root.querySelector('[data-ics]')?.addEventListener('click', () => {
      const svc = SERVICES.find(s => s.key === state.serviceKey);
      const ics = buildICS({
        summary: `${svc.title} — HomeMas`,
        description: `Afspraak van ${state.duration} minuten bij HomeMas (Nienke Poiesz).`,
        location: 'Tweede Walstraat 85, Nijmegen',
      ...
      a.href = url;
      a.download = 'homemas-afspraak.ics';
      a.click();
