/**
 * Booking modal — B-KwieK Sport- & Herstelmassages
 *
 * Architecture (per /skills/booking-system/SKILL.md):
 *   1. On open: fetch /api/event-types, build slug → ID map
 *   2. Step "service":   user picks sport- or herstelmassage
 *   3. Step "duration":  60 vs 90 min
...
(function () {
  ...
  // ─── SHOP_CONFIG ──────────────────────────────────────────────────
  // Per brief: shopName, phoneDisplay, phoneE164, domain, addressICS, hours, services, price.
  const SHOP_CONFIG = {
    shopName: 'B-KwieK Sport- & Herstelmassages',
    phoneDisplay: '[PLACEHOLDER: telefoon]',     // niet vermeld op bron-site
    phoneE164: '[PLACEHOLDER]',                  // niet vermeld op bron-site
    domain: 'b-kwiek.nl',
    addressICS: '[PLACEHOLDER: adres], Zwolle-Stadshagen', // alleen wijk bekend van bron-site
  };

  const SHOP_HOURS = {
    // 0=Sun … 6=Sat. null = closed. Slots are quoted in local Europe/Amsterdam time.
    // [PLACEHOLDER: openingstijden — niet op bron-site. Default ma–vr 08:00–18:00 — bevestigen met klant.]
    0: null,
  ...
  // Each service has up to two durations. Slugs match Cal.com Event Types:
  // sport-60, sport-90, herstel-60, herstel-90.
  const SERVICES = [
    { key: 'sport',    title: 'Sportmassage',   desc: 'Activerende technieken vóór inspanning',         durations: [60, 90] },
    { key: 'herstel',  title: 'Herstelmassage', desc: 'Ontspannende technieken voor spierherstel',      durations: [60, 90] },
  ];

  // 60-min hele rug = €70 (bron). 30/90-min varianten: indicatief — bevestigen met klant.
  const PRICE = { 30: '€40', 60: '€70', 90: '€95' };
  ...
  const buildICS = ({ summary, description, location, start, durationMin }) => {
    ...
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//b-kwiek.nl//Booking//NL',
      'BEGIN:VEVENT',
      `UID:${Date.now()}@b-kwiek.nl`,
      `DTSTAMP:${dt(new Date())}`,
      `DTSTART:${dt(start)}`,
      `DTEND:${dt(end)}`,
  ...
  async function wireTime(root) {
    ...
    if (!eventTypeId) {
      status.textContent = 'Online boeken nog niet geactiveerd voor deze massage — neem contact op via info@b-kwiek.nl.';
    ...
    try {
      ...
      status.textContent = 'Kon beschikbaarheid niet ophalen. Probeer opnieuw of bel 06 542 005 28.';
      console.warn(e);
    }
  }
    ...
    return `
      ...
      <form class="space-y-3" data-form>
        ...
        <div>
          ...
          <textarea name="notes" rows="3" class="w-full bg-bg-deep border border-border-subtle rounded-[12px] px-4 py-3 text-ink-primary placeholder:text-ink-faint focus:outline-none focus:border-gold transition-colors" placeholder="Wat speelt er? Sportblessure, herstel na inspanning, eerste keer?"></textarea>
        </div>
  ...
  function wireDetails(root) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        errEl.textContent = err.message || 'Kon de boeking niet vastleggen. Probeer opnieuw of mail info@b-kwiek.nl.';
  ...
  function viewConfirm() {
    ...
    return `
      <div class="text-center py-2">
        ...
        <p class="text-ink-muted mb-7 max-w-md mx-auto">Je krijgt een bevestiging in je inbox. Tot ziens bij B-KwieK in Zwolle-Stadshagen.</p>
  ...
  function wireConfirm(root) {
    root.querySelector('[data-ics]')?.addEventListener('click', () => {
      const svc = SERVICES.find(s => s.key === state.serviceKey);
      const ics = buildICS({
        summary: `${svc.title} — B-KwieK`,
        description: `Afspraak van ${state.duration} minuten bij B-KwieK Sport- & Herstelmassages.`,
      ...
      a.href = url;
      a.download = 'b-kwiek-afspraak.ics';
      a.click();
  ...
  window.BKwieK_Booking = { open: openModal, close: closeModal };
})();
