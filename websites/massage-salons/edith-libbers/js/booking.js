/**
 * Booking modal — Edith Libbers Sportmassage & Voetreflextherapie
 *
 * Architecture (per /skills/booking-system/SKILL.md):
 *   1. On open: fetch /api/event-types, build slug → ID map
 *   2. Step "service":   user picks one of 4 massage types
...
(function () {
  ...
  // ─── SHOP_CONFIG ──────────────────────────────────────────────────
  // Edit ONLY this block when reusing this template for a new shop.
  const SHOP_CONFIG = {
    shopName:    'Edith Libbers Sportmassage & Voetreflextherapie',
    phoneDisplay:'06 18 18 17 96',
    phoneE164:   '+31618181796',
    domain:      'edithlibbers.nl',
    email:       'PLACEHOLDER: verify e-mailadres',
    addressICS:  'Groesbeekseweg 204, 6523 PD Nijmegen',
    // PLACEHOLDER: bron-site noemt geen openingstijden. Aanname: ma-vr 09:00-18:00.
    hours: {
      0: null,
    ...
    services: [
      { key: 'sport',        title: 'Sportmassage',                desc: 'Doorbloeding, herstel en blessurepreventie', durations: [45, 60, 90] },
      { key: 'ontspanning',  title: 'Ontspanningsmassage',         desc: 'Klassiek kneden met natuurlijke olie',       durations: [45, 60, 90] },
      { key: 'voetzone',     title: 'Voetzonereflextherapie',      desc: 'Activeert reflexzones op de voeten',         durations: [60] },
      { key: 'stoel',        title: 'Stoelmassage',                desc: 'Door de kleding heen — kantoor of event',    durations: [15, 30], fixed: false },
    ],
    price: { 15: '€17,50', 30: '€33,50', 45: '€49', 60: '€65', 90: '€95' },
  };
  // ──────────────────────────────────────────────────────────────────
  ...
  // Slugs are computed as `${service.key}-${duration}`, e.g. "sport-60".
    ...
    const cards = SERVICES.map(s => {
      const priceLine = FREE_KEYS.has(s.key)
        ...
        : s.durations.length === 1
          ? PRICE[s.durations[0]]
          : `${PRICE[s.durations[0]]} — ${PRICE[s.durations[s.durations.length - 1]]}`;
      return `
  ...
  function viewDuration() {
    ...
    return `
      ...
      <div class="grid sm:grid-cols-${Math.min(svc.durations.length, 3)} gap-3">
        ${svc.durations.map(d => `
  ...
  function viewDate() {
    ...
    return `
      ...
      <p class="text-sm text-ink-muted mb-6">Ma t/m vr · 09:00 — 18:00. Weekend en feestdagen op aanvraag.</p>
  ...
  function wireDate(root) {
    ...
    root.querySelector('[data-back]')?.addEventListener('click', () => {
      const svc = SERVICES.find(s => s.key === state.serviceKey);
      state.step = (svc?.durations.length === 1) ? 'service' : 'duration';
      render();
    });
  }
    ...
    return `
      ...
      <form class="space-y-3" data-form>
        ...
        <div>
          ...
          <textarea name="notes" rows="3" class="w-full bg-bg-deep border border-border-subtle rounded-[12px] px-4 py-3 text-ink-primary placeholder:text-ink-faint focus:outline-none focus:border-gold transition-colors" placeholder="Wat speelt er? Sportmassage voor specifieke klacht? Eerste keer bij Edith?"></textarea>
        </div>
  ...
  function viewConfirm() {
    ...
    return `
      <div class="text-center py-2">
        ...
        <p class="text-ink-muted mb-7 max-w-md mx-auto">Je krijgt een bevestiging in je inbox. Tot ziens bij de praktijk aan de ${SHOP_CONFIG.addressICS}.</p>
  ...
  function wireConfirm(root) {
    root.querySelector('[data-ics]')?.addEventListener('click', () => {
      ...
      const ics = buildICS({
        ...
        description: `Afspraak van ${state.duration} minuten bij Edith Libbers.`,
      ...
      a.href = url;
      a.download = 'edith-libbers-massage.ics';
      a.click();
  ...
  window.EL_Booking = { open: openModal, close: closeModal };
})();
