/**
 * Booking modal — Massages InBalans (Gaiane Abrahamian, Nijmegen)
 *
 * Architecture (per /skills/booking-system/SKILL.md):
 *   1. On open: fetch /api/event-types, build slug → ID map
 *   2. Step "service":   user picks one of the massage forms
 *   3. Step "duration":  60 / 75 / 90 min (skipped for fixed-duration services)
 *   4. Step "date":      a two-week date strip
...
(function () {
  ...
  // ─── SHOP CONFIG ──────────────────────────────────────────────────
  // All shop-specific values live in this single block. The rest of the
  // file references these via SHOP_CONFIG.* and never hard-codes
  // anything else about the practice.
  const SHOP_CONFIG = {
    shopName:      'Massages InBalans',
    practitioner:  'Gaiane Abrahamian',
    phoneDisplay:  '06 140 477 00',
    phoneE164:     '+31614047700',
    email:         'praktijk@massagesinbalans.nl',
    domain:        'massagesinbalans.nl',
    addressICS:    'Groesbeekseweg 246b, 6523 PJ Nijmegen',
    // PLACEHOLDER: opening hours not published on source-site — verify with Gaiane.
    // 0=Sun … 6=Sat. null = closed. Slots are in Europe/Amsterdam local time.
    hours: {
      0: null,
    ...
    // Each service has up to three durations. The slug is what gets created
    // as an Event Type in Cal.com. The combination `${key}-${duration}`
    // must match exactly with the slug field in Cal.com.
    services: [
      { key: 'ontspanning',   title: 'Ontspanningsmassage',                    desc: 'Rug, nek, schouders, benen, voeten en armen — met basisolie.', durations: [60, 75, 90] },
      { key: 'peace-calming', title: 'Peace & Calming — UNIEK',                desc: 'Exclusieve luxe ontspanningsmassage met hot stone, etherische oliën en warme kompresdoeken.', durations: [60, 75, 90], premium: true },
      { key: 'voetreflex',    title: 'Voetreflexmassage',                      desc: 'Inclusief warm voetbad met magnesium, etherische olie en ademhalingsbegeleiding.', durations: [45, 60, 75] },
      { key: 'metamorfose',   title: 'Metamorfosemassage',                     desc: 'Energetische massage die op het onderbewuste werkt en blokkades wegneemt.', durations: [60], fixed: true },
    ],
    // Per-service price map in cents-of-euro. Lookup as price[key][duration].
    // Keep in sync with the tariff strip on behandelingen.html.
    price: {
      'ontspanning':   { 60: '€85',  75: '€100', 90: '€115' },
      'peace-calming': { 60: '€100', 75: '€115', 90: '€130' },
      'voetreflex':    { 45: '€65',  60: '€80',  75: '€100' },
      'metamorfose':   { 60: '€85' },
    },
    freeKeys: new Set([]),
  };
  ...
  const SHOP_HOURS = SHOP_CONFIG.hours;
  const SERVICES   = SHOP_CONFIG.services;
  const FREE_KEYS  = SHOP_CONFIG.freeKeys;

  const priceFor = (key, dur) => {
    if (FREE_KEYS.has(key)) return 'gratis';
    return SHOP_CONFIG.price[key]?.[dur] ?? '';
  };
  const priceRange = (svc) => {
    if (FREE_KEYS.has(svc.key)) return 'gratis';
    if (svc.fixed) return priceFor(svc.key, svc.durations[0]);
    const lo = priceFor(svc.key, svc.durations[0]);
    const hi = priceFor(svc.key, svc.durations[svc.durations.length - 1]);
    return `${lo} — ${hi}`;
  };
  ...
  const buildICS = ({ summary, description, location, start, durationMin }) => {
    ...
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      `PRODID:-//${SHOP_CONFIG.domain}//Booking//NL`,
      'BEGIN:VEVENT',
      `UID:${Date.now()}@${SHOP_CONFIG.domain}`,
      `DTSTAMP:${dt(new Date())}`,
      `DTSTART:${dt(start)}`,
      `DTEND:${dt(end)}`,
  ...
  function viewDuration() {
    ...
    return `
      ...
      <div class="grid sm:grid-cols-${svc.durations.length} gap-3">
        ${svc.durations.map(d => `
          <button class="text-left bg-bg-deep border border-border-subtle rounded-[16px] p-6 hover:border-gold/60 hover:bg-bg-elevated transition-all" data-dur="${d}">
            ...
            <p class="text-[11px] tracking-kicker uppercase text-gold font-semibold">${priceFor(svc.key, d)}</p>
          </button>
        `).join('')}
      </div>
  ...
  function viewDate() {
    ...
    return `
      ...
      <p class="text-sm text-ink-muted mb-6">Alleen op afspraak — kies een open dag.</p>
  ...
  async function wireTime(root) {
    ...
    if (!eventTypeId) {
      status.textContent = `Online boeken nog niet geactiveerd voor deze massage — bel of WhatsApp ${SHOP_CONFIG.phoneDisplay}.`;
    ...
    try {
      ...
      status.textContent = `Kon beschikbaarheid niet ophalen. Probeer opnieuw of bel ${SHOP_CONFIG.phoneDisplay}.`;
      console.warn(e);
    }
  }
    ...
    return `
      ...
      <form class="space-y-3" data-form>
        ...
        <div>
          ...
          <textarea name="notes" rows="3" class="w-full bg-bg-deep border border-border-subtle rounded-[12px] px-4 py-3 text-ink-primary placeholder:text-ink-faint focus:outline-none focus:border-gold transition-colors" placeholder="Wat speelt er? Eerste keer bij ${SHOP_CONFIG.practitioner.split(' ')[0]}? Vraag of voorkeur?"></textarea>
        </div>
  ...
  function wireDetails(root) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        errEl.textContent = err.message || `Kon de boeking niet vastleggen. Probeer opnieuw of bel ${SHOP_CONFIG.phoneDisplay}.`;
  ...
  function viewConfirm() {
    ...
    return `
      <div class="text-center py-2">
        ...
        <p class="text-ink-muted mb-7 max-w-md mx-auto">Je krijgt een bevestiging in je inbox. Tot ziens bij de praktijk aan de ${SHOP_CONFIG.addressICS.split(',')[0]}.</p>
  ...
  function wireConfirm(root) {
    root.querySelector('[data-ics]')?.addEventListener('click', () => {
      const svc = SERVICES.find(s => s.key === state.serviceKey);
      const firstName = SHOP_CONFIG.practitioner.split(' ')[0];
      const ics = buildICS({
        summary: `${svc.title} — ${SHOP_CONFIG.shopName}`,
        description: `Afspraak van ${state.duration} minuten bij ${firstName}.`,
        location: SHOP_CONFIG.addressICS,
      ...
      a.href = url;
      a.download = `${SHOP_CONFIG.domain.replace(/\./g, '-')}.ics`;
      a.click();
  ...
  window.InBalans_Booking = { open: openModal, close: closeModal };
})();
