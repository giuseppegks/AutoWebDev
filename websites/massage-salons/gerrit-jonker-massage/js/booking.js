/**
 * Booking modal — Gerrit Jonker Massage
 *
...
(function () {
  ...
  const SERVICES = [
    { key: 'intuitief',  title: 'Intuïtieve massage',         desc: 'Aandacht, verbinding, harmonie', durations: [60, 90] },
    { key: 'sport',      title: 'Sportmassage',               desc: 'Doorbloeding, herstel, blessures', durations: [60, 90] },
    { key: 'mer',        title: 'Myofasciale Release (MER)',  desc: 'Diepe druk op bindweefsel',       durations: [60, 90] },
    { key: 'psoas',      title: 'Psoas-massage',              desc: 'Diepe bekkenspier, stress, lage rug', durations: [60, 90] },
    { key: 'cranio',     title: 'Craniosacrale ontspanning',  desc: 'Zachte technieken, diepe rust',   durations: [60, 90] },
    { key: 'stoel',      title: 'Stoelmassage',               desc: 'Volledig gekleed, 30 minuten',     durations: [30], fixed: true },
    { key: 'intake',     title: 'Intakegesprek bij kanker',   desc: 'Vrijblijvend kennismaken (gratis)',durations: [30], fixed: true, free: true },
  ];

  const PRICE = { 30: '€37,50', 60: '€65', 90: '€90' };
  const FREE_KEYS = new Set(['intake']);
  ...
  const buildICS = ({ summary, description, location, start, durationMin }) => {
    ...
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//gerritjonkermassage.nl//Booking//NL',
      'BEGIN:VEVENT',
      `UID:${Date.now()}@gerritjonkermassage.nl`,
      `DTSTAMP:${dt(new Date())}`,
      `DTSTART:${dt(start)}`,
      `DTEND:${dt(end)}`,
  ...
  async function wireTime(root) {
    ...
    if (!eventTypeId) {
      status.textContent = 'Online boeken nog niet geactiveerd voor deze massage — bel of WhatsApp 06 542 005 28.';
    ...
    return `
      ...
      <form class="space-y-3" data-form>
        ...
        <div>
          ...
          <textarea name="notes" rows="3" class="w-full bg-bg-deep border border-border-subtle rounded-[12px] px-4 py-3 text-ink-primary placeholder:text-ink-faint focus:outline-none focus:border-gold transition-colors" placeholder="Wat speelt er? Eerste keer bij Gerrit? Vraag voor de intake?"></textarea>
        </div>
  ...
  function wireDetails(root) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        errEl.textContent = err.message || 'Kon de boeking niet vastleggen. Probeer opnieuw of bel 06 542 005 28.';
  ...
  function viewConfirm() {
    ...
    return `
      <div class="text-center py-2">
        ...
        <p class="text-ink-muted mb-7 max-w-md mx-auto">Je krijgt een bevestiging in je inbox. Tot ziens bij de praktijk aan de Cees de Jongstraat 6.</p>
  ...
  function wireConfirm(root) {
    root.querySelector('[data-ics]')?.addEventListener('click', () => {
      const svc = SERVICES.find(s => s.key === state.serviceKey);
      const ics = buildICS({
        summary: `${svc.title} — Gerrit Jonker Massage`,
        description: `Afspraak van ${state.duration} minuten bij Gerrit Jonker.`,
        location: 'Cees de Jongstraat 6, Nijmegen-Oost',
      ...
      a.href = url;
      a.download = 'gerrit-jonker-massage.ics';
      a.click();
