// Café Trianon — site interactions
...
(function todayHours() {
  ...
  // Lunch hours: ma–vr 11:30–14:30, weekend 13:00–16:00
  if (day === 0 || day === 6) {
    label.textContent = 'Lunch vandaag';
    hours.textContent = '13:00 — 16:00';
  } else {
    label.textContent = 'Lunch vandaag';
    hours.textContent = '11:30 — 14:30';
  }
})();
...
if (statusBadge) {
  // Lunch hours: ma–vr 11:30–14:30, weekend 13:00–16:00.
  // Avondopening alleen bij concerten — niet automatisch te bepalen.
  const day = new Date().getDay(); // 0=zo, 6=za
  ...
  if (txt && dot) {
    if (isOpen) {
      txt.textContent = 'Lunch geopend';
      dot.style.background = '#4ade80';
      dot.style.boxShadow = '0 0 0 4px rgba(74,222,128,0.18)';
    } else {
      txt.textContent = 'Check de agenda';
      dot.style.background = '#D89A3E';
      dot.style.boxShadow = '0 0 0 4px rgba(216,154,62,0.22)';
    }
  }
}
...
document.querySelectorAll('[data-mailto-form]').forEach((form) => {
  form.addEventListener('submit', (e) => {
    ...
    const lines = [
      ...
      get('bericht') || '—',
      '',
      '— Verstuurd via trianonnijmegen.nl'
    ];
    const body = lines.join('\n');
    const url = `mailto:info@trianonnijmegen.nl?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
