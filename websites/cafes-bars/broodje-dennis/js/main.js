// Billy's Coffee — site interactions
...
// Broodje Dennis: Do t/m Zo 10:00–17:00, Ma t/m Wo gesloten
(function todayHours() {
  ...
  // Open days: 4=do, 5=vr, 6=za, 0=zo
  if (day === 4 || day === 5 || day === 6 || day === 0) {
    label.textContent = 'Vandaag open';
    hours.textContent = '10:00 — 17:00';
  } else {
    label.textContent = 'Vandaag';
    hours.textContent = 'Gesloten (do–zo open)';
  }
})();
...
if (statusBadge) {
  // Hours: do-zo 10:00-17:00, ma-wo gesloten
  const day = new Date().getDay(); // 0=zo
  const minutesNow = new Date().getHours() * 60 + new Date().getMinutes();
  let openMin = null, closeMin = 17 * 60;
  if (day === 4 || day === 5 || day === 6 || day === 0) {
    openMin = 10 * 60;
  } else {
    openMin = null;
  ...
  const isOpen = openMin !== null && minutesNow >= openMin && minutesNow < closeMin;
...
document.querySelectorAll('[data-mailto-form]').forEach((form) => {
  form.addEventListener('submit', (e) => {
    ...
    const subject = `Cateringaanvraag — ${get('naam')} (${get('personen')})`;
    const lines = [
      `Naam: ${get('naam')}`,
      `E-mail: ${get('email')}`,
      ...
      get('bericht') || '—',
      '',
      '— Verstuurd via broodje-dennis.nl'
    ];
    const body = lines.join('\n');
    // PLACEHOLDER e-mailadres — vervang dit door het echte adres van Broodje Dennis
    const url = `mailto:info@broodjedennis.nl?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
