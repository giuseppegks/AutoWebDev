...
(function todayHours() {
  ...
  if (day === 2) {
    ...
    hours.textContent = 'Gesloten (di)';
  } else if (day === 0) {
    label.textContent = 'Vandaag open';
    hours.textContent = '9:30 — 16:30';
  } else {
    label.textContent = 'Vandaag open';
    hours.textContent = '8:30 — 16:30';
  }
})();
...
if (statusBadge) {
  // Hours: ma + wo-za 8:30-16:30, zo 9:30-16:30, di gesloten
  const day = new Date().getDay(); // 0=zo
  const minutesNow = new Date().getHours() * 60 + new Date().getMinutes();
  let openMin = null, closeMin = 16 * 60 + 30;
  if (day === 2) { openMin = null; }            // di
  else if (day === 0) { openMin = 9 * 60 + 30; } // zo
  else { openMin = 8 * 60 + 30; }                // ma + wo–za
